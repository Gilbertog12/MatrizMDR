...
@Component({
  selector: 'app-rkyriesgoresidualtable',
  templateUrl: './rkyriesgoresidualtable.component.html',
  styleUrls: ['./rkyriesgoresidualtable.component.scss']
})

export class RkyriesgoresidualtableComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<RkyriesgoresidualtableComponent>,
  ...
  deseleccionarTodo(item: any) { 
    ...
    this.riesgoResidualModel.probabilidadId = '';
    if ( item.selected === true ) {
      this.riesgoResidualModel.probabilidadId = item.id;
    }
  }
      ...
      .subscribe(
        (data) => {
          ...
          if (result) {
            ...
            //alert(this.riesgoResidualModel.probabilidadId);

            data.data.forEach( (element) => {
              if ( element.atts.length > 0) {

                  this.dataList.push({
                    selected: element.atts[1].value.trim() === this.riesgoResidualModel.probabilidadId ? true : false,
  ...
  guardar() {
    ...
    _atts.push({ name: 'riesgoResidualS', value: '' });
